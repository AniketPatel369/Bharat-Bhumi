...
export function setupRoutes(app: express.Application, storage: IStorage) {
  ...
  const autoEndTurn = async (roomId: string, reason: "actionTimeout" | "autoRoll") => {
    ...
    await storage.updatePlayer(roomId, currentPlayer.id, { hasRolledThisTurn: false, consecutiveDoubles: 0, pendingDoubleDecision: false });
        ...
        switch (message.type) {
          ...
          case 'rollDice':
            try {
              ...
              if (goJailByTripleDouble) {
                ...
                await storage.updatePlayer(ws.roomId, ws.playerId, { hasRolledThisTurn: false });
                scheduleRollTimer(ws.roomId);
              } else {
          ...
          case 'buyProperty':
            try {
              ...
              if (!activePlayer || activePlayer.money < price) {
                ws.send(JSON.stringify({
                  type: 'error',
              ...
              await storage.updatePlayer(ws.roomId, ws.playerId, {
                money: activePlayer.money - price,
